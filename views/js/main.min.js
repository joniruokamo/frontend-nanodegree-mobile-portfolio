...
function changePizzaSizes(size){var rpc=document.querySelectorAll(".randomPizzaContainer");var dx=determineDx(rpc[0],size);var newwidth=(rpc[0].offsetWidth+dx)+'px';for(var i=0;i<rpc.length;i++){rpc[i].style.width=newwidth}}
...
function updatePositions(){frame++;window.performance.mark("mark_start_frame");var items=document.querySelectorAll('.mover');var scrollTop=document.documentElement.scrollTop||document.body.scrollTop;for(var i=0;i<items.length;i++){var phase=Math.sin((scrollTop/1250)+(i%5));items[i].style.left=items[i].basicLeft+100*phase+'px'}
...
window.addEventListener('scroll',updatePositions,{passive:!0});document.addEventListener('DOMContentLoaded',function(){var cols=8;var s=256;for(var i=0;i<200;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.height="100px";elem.style.width="73.333px";elem.basicLeft=(i%cols)*s;elem.style.top=(Math.floor(i/cols)*s)+'px';document.querySelector("#movingPizzas1").appendChild(elem)}
updatePositions()},{passive:!0})